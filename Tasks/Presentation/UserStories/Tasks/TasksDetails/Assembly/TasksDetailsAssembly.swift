//
//  TasksDetailsAssembly.swift
//  Tasks
//
//  Created by Alexey on 05/04/2019.
//  Copyright © 2019 Alexey. All rights reserved.
//

import EasyDi
import Foundation

// swiftformat:disable redundantSelf

final class TasksDetailsAssembly: Assembly {
    private lazy var servicesAssembly: ServicesAssembly = context.assembly()
    private lazy var dateFormattersAssembly: DateFormattersAssembly = context.assembly()

    func inject(into view: TasksDetailsViewController) {
        defineInjection(key: "view", into: view) {
            $0.viewModel = self.viewModel
            return $0
        }
    }

    private var view: TasksDetailsViewController {
        return definePlaceholder()
    }

    var viewModel: TaskViewModel {
        return define(
            init: TaskViewModelImp(
                tasksService: self.servicesAssembly.tasksService,
                dateRangeFormatter: self.dateFormattersAssembly.dateRangeFormatter
            )
        )
    }
}
